...
for model in unsloth/Llama-3.2-1B-Instruct unsloth/Llama-3.2-3B-Instruct unsloth/Meta-Llama-3.1-8B-Instruct Qwen/Qwen2.5-7B-Instruct
do
    # For experiment path take only the model name, not the path
    # Assumes only one '/' in the model name
    experiment_name=$(echo $model | cut -d '/' -f 2)---template-ensemble
